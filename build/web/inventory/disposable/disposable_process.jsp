...
<html>
   ...
   <link rel="stylesheet" href="style_form.css">
    ...
    <body>
        <%
           ...
           name1 = session.getAttribute("name").toString();
       %>
        ...
        <%@ page import="pkg.connect" %>
         ...
         <%
            String name = request.getParameter("disposable_name").toUpperCase();
            int qty = Integer.parseInt(request.getParameter("quantity"));
            
            connect conn = new connect();
            Connection connection = conn.createConnection();
            PreparedStatement stmt = connection.prepareStatement("insert into disposable (disposable_name , quantity) values (?,?)");
            stmt.setString(1, name);
            stmt.setInt(2, qty);
            
            
            String sql = "select disposable_name from disposable where disposable_name ='"+name+"'";
         ...
         <script>
                {
                swal("Error","Disposable already exist", "error");
                }
                 </script>
            <%
             }
   ...
   <section class="flex">
      ...
      <div class="profile">
         ...
         <h3 class="name"><%out.println(name1);%></h3>
