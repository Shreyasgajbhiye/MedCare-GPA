    ...
    <body>
        ...
        <%
            ...
            try{
                ...
                PreparedStatement stmt = connection.prepareStatement("select * from doctor where email = ? and password = ?");
                stmt.setString(1,email);
                stmt.setString(2,pass);
                
                PreparedStatement stmt1 = connection.prepareStatement("select * from doctor where email = ?");
            ...
            <%
                ...
                <script>
                {
                swal("Error",<%out.println(e.toString());%>, "error");
                }
                 </script>
            <%
            }
            //
             ...
             connect conn = new connect();
                Connection connection = conn.createConnection();
                PreparedStatement stmt = connection.prepareStatement("select * from admin where email = ? and password = ?");
                stmt.setString(1,email);
                stmt.setString(2,pass);
                
                
                PreparedStatement stmt1 = connection.prepareStatement("select * from admin where email = ?");
                stmt1.setString(1,email);
                ResultSet rs1 = stmt1.executeQuery();
                while(rs1.next()){
                first_name = rs1.getString("first_name");
